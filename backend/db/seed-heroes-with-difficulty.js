...
const seedHeroes = async () => {
  ...
  try {
    ...
    // Truncate and reseed heroes table
    await client.query('TRUNCATE TABLE heroes CASCADE');
    console.log('Truncated heroes table');

    // Insert all heroes with difficulty ratings
    ...
    console.log(`✓ Successfully seeded ${heroes.length} heroes with difficulty ratings!`);
    await client.query('SELECT COUNT(*) as count FROM heroes');
  } catch (err) {
    console.error('Error seeding heroes:', err);
  } finally {
    await client.end();
    process.exit(0);
  }
