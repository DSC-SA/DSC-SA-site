...
export default function HeroCard({ hero }) {
  ...
  return (
    <Link to={`/heroes/${hero.id}`} className="group block h-full w-full">
      <TiltCard className="hero-glow relative flex h-full w-full flex-col overflow-hidden rounded-2xl border border-brand-line bg-brand-snow transition-colors duration-300 group-hover:border-brand-blue">
        {/* Portrait fill */}
